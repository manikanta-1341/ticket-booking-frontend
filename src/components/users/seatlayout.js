...
import {
    Button, Grid, TableBody, Table, TableRow, TableCell, Typography, Box, CardContent,
    Stack,  Card, Divider, TableHead
} from "@mui/material";
...
export default function SeatLayoutComp() {
    ...
    window.onload = ()=>{ return window.location.assign('/movies') }
   
    const theaterDetails = useSelector(state => state.theaterDetails)[0]
    const occupiedSeats = theaterDetails.theater_details.cinemas[0].selected[0]
    ...
    useEffect(() => {
        ...
        let alphas_index = alphas.indexOf(row_starting_letter[row_starting_letter_index])
        let dummyVar = []
        let res = []
        for (var i = 0; i <= no_of_seats; i++) {
            if (dummyVar.length === row_range * row_capacity) {
                ...
                res.push(dummyRes)
                if (res.length === row_starting_letter.length) { break; }
                ticket_index++
            }
            if (i === row_capacity) {
                alphas_index++
                ...
                if (res.length > 0 && dummyVar.length === 0) {
                    ...
                    alphas_index = alphas.indexOf(row_starting_letter[row_starting_letter_index])
                }
            }
            dummyVar.push({
                id: i,
    ...
    }, [movie_time, theater_details.availabilityTickets, theater_details.rowCapacity, theater_details.rowNames, theater_details.rowRanges, theater_details.screens, theater_details.showTimes, theater_details.ticketRanges])
    ...
    const handleChange = (i, sub_i, e_i) => {
        ...
        let total_value = dummySelected.length>0 &&dummySelected.map((e) => { return e.price }).reduce((acc, cv) => parseInt(acc) + parseInt(cv))
        ...
        <Box>
                ...
                <Grid item xs={12}>
                    {
                        seats.map((arr, i) => {
                            return (
                                <Box key={i}>
                                    <Table  size="small" padding="none" sx={{ "&.MuiTable-root": { width: 480, mt: "2%", mx: "auto" } }}>
                                        <TableHead >
                                            <TableRow >
                                                <TableCell colSpan={global_row_capacity} sx={{ "&.MuiTableCell-root": { borderBottom: "none" }}}>
                                                    <Stack direction="row"  color="text.secondary">
                                                        <Typography sx={{ textTransform: "capitalize" }}>{reverse_ticketName.reverse()[i]}-Rs&nbsp;</Typography>
                                                        <Typography>{reverse_ticketRanges.reverse()[i]}</Typography>
                                                    </Stack>
                                                    <Divider />
                                                </TableCell>
                                            </TableRow>
                                                ...
                                                arr.map((sub_arr, sub_i) => {
                                                    return (
                                                        <TableRow key={sub_i}>
                                                            {sub_arr.map((e, e_i) => {
                                                                return (
                                                                    <TableCell key={e.seatNo} sx={{ "&.MuiTableCell-root": { borderBottom: "none", p: "3px" } }}>
                                                                        {
                                                                            e_i === 0 ?
                                                                                
                                                                                    <Typography sx={{display:"inline-flex",gap:"20px",alignItems:"center", textTransform: "capitalize", color: "text.secondary" }}>
                                                                                        {e.seatNo.replace(new RegExp(/\d/g), "")}
                                                                                    <Button
                                                                                        variant={"outlined"}
                                                                                        disabled={theater_details.selected?
                                                                                            occupiedSeats.seats.includes(e.seatNo) && 
                                                                                            e.seatNo === occupiedSeats.seats.split(",")[occupiedSeats.seats.split(",").indexOf(e.seatNo)] &&
                                                                                           parseInt( e.price) === occupiedSeats.prices[occupiedSeats.prices.indexOf(parseInt(e.price))] &&
                                                                                           theaterDetails.movie_details.selected
                                                                                           ?true:false:false}  
                                                                                        disableRipple
                                                                                        onMouseOver={() => handleMouseOver(i, sub_i, e_i)}
                                                                                        onMouseOut={() => handleMouseOut(i, sub_i, e_i)}
                                                                                        ...
                                                                                        sx={{
                                                                                            ...
                                                                                            backgroundColor: e.selected ? "#2e7d32" : "transparent",
                                                                                            color: e.selected || e.hover ? "white" : "",
                                                                                            "&:hover": {
                                                                                                backgroundColor: e.selected || e.hover ? "#2e7d32" : "inherit"
                                                                                            }
                                                                                        }}>{e.seatNo.replace(new RegExp(/\D/g), "")}
                                                                                    </Button>
                                                                                    </Typography>
                                                                                ...
                                                                                : e_i === 1 ?
                                                                                    ...
                                                                                    <Box sx={{ display: "flex", justifyContent: "space-between", borderBottom: 0 }}>
                                                                                        <Button
                                                                                            variant={"outlined"}
                                                                                            disabled={theater_details.selected?
                                                                                                occupiedSeats.seats.includes(e.seatNo) && 
                                                                                    ...
                                                                                    e_i === 7 ?
                                                                                        ...
                                                                                        <Box sx={{ display: "flex", justifyContent: "space-between", borderBottom: 0 }}>
                                                                                            <Button
                                                                                                ...
                                                                                                }}>{e.seatNo.replace(new RegExp(/\D/g), "")}</Button>
                                                                                            <Box>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</Box>
                                                                                        </Box>
                                                                                        : <Button
                    ...
                    <Box sx={{ mt: 4, width: "100%", mx: "auto", textAlign: "center" }}>
                        ...
                        <Box width={350} sx={{ mt: 1, mx: "auto", boxShadow: "0px 0px 4px 2px #009eff7d", transform: "matrix(1, 0, 1, 1, 20, 4)" }}></Box>
                    </Box>
                </Grid>
            ...
            {selected_seats.length === 0 ?
                    ...
                    <CardContent>
                        <Grid container spacing={4} justifyContent="center" alignItems="center">
                            <Grid item sx={{ display: "flex", alignItems: "center" }}>
                                <CheckBoxOutlineBlank color="disabled" sx={{ display: "inline-flex" }}></CheckBoxOutlineBlank>
                                <Typography>Sold</Typography>
                            </Grid>
                            <Grid item sx={{ display: "flex", alignItems: "center" }}>
                                <Box sx={{ display: "inline-flex", width: 20, height: 20, bgcolor: "green", borderRadius: "3px" }} />
                                <Typography>Selected</Typography>
                            </Grid>
                            <Grid item sx={{ display: "flex", alignItems: "center" }}>
                                <CheckBoxOutlineBlank color="success" sx={{ display: "inline-flex" }} ></CheckBoxOutlineBlank>
                ...
                <Box sx={{ textAlign: "right", mt: "6%" }}>
                    <Button fullWidth sx={{ position: "sticky", bottom: 0 }} onClick={() => handleCheckout()} variant="contained" color="error">Proceed&nbsp;To&nbsp;Pay</Button>
                </Box>}
        </Box>
    );
}